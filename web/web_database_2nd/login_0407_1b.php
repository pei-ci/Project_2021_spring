<?php
$GLOBALS['SERVER_VERSION'] = '0407_1b'; 
...
$GLOBALS['SPECIAL_PUZZLE_POINT'] = 10;
...
function dealing_special_request($mysqli,$login_certification,$value,$set){
    ...
    if($set=='1'){
        $respond['write_to_team'] = 'false';
        if(update_team_point($mysqli,$login_certification,$GLOBALS['SPECIAL_PUZZLE_POINT'])){
            $respond['write_to_team'] = 'true';
        }
    }
...
function get_personal_point($mysqli,$login_certification,$number){
    ...
    $query_check = "SELECT used,unused1,unused2,unused3,unused4,unused5,unused6,special   
    FROM user, map".$selection;
    $result_check = mysqli_query($mysqli, $query_check) or die(mysqli_error($mysqli));
    $num_row_check = mysqli_num_rows($result_check);
    $row_check = mysqli_fetch_array($result_check);
    if($num_row_check!=1){
    ...
    $point = 0;
    $point += ((int)$row_check['used']) * $GLOBALS['USED_PUZZLE_POINT'];
    for($x = 1; $x <= 6; $x++) {
        $point += (int)$row_check['unused'.$x] * $GLOBALS['UNUSED_PUZZLE_POINT'];
    }
    for($x = 0; $x < 3; $x++) {
        $point += (int)substr($row_check['special'],$x*2,1) * $GLOBALS['SPECIAL_PUZZLE_POINT'];
    }
    return $point;
}
