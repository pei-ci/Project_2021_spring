...
function dealing_login_request($mysqli,$email,$password){    
    $query = "SELECT user.userid, password, log.login_count FROM user,log WHERE user.number='$email' AND user.userid=log.userid";
    ...
    if ($num_row == 1) {
        if (password_verify($password, $row['password'])){
            ...
            if($result){
                ...
                $respond['login_time'] = $row['login_count'];               
                $respond["sucess"] = 'true';
            }
        }else{
...
function dealing_register_request($mysqli,$email,$password,$name,$department,$nickname){
    ...
    //get activity from activity table
    $query_activity = "SELECT point,add_point FROM activity WHERE stu_id='$email'";
    $result_activity = mysqli_query($mysqli, $query_activity) or die(mysqli_error($mysqli));
    $num_row_activity = mysqli_num_rows($result_activity);    
    if($num_row_activity>0){
        // set userid to activity table (replace NULL)
        $query_put = "UPDATE activity SET userid='$id' WHERE stu_id='$email'";
        $result_put = mysqli_query($mysqli, $query_put) or die(mysqli_error($mysqli));
    }

    //register to map table
    $query_register_map = generate_register_map_command($num_row_activity,$result_activity,$id);
...
function generate_register_map_command($num_row_activity,$result_activity,$id){
    $emergency = $GLOBALS['EMERGENCY_DEFAULT_VALUE'];
    $point = 0;
    $puzzle_value = [];

    for($x = 1; $x < 7; $x++){
        $puzzle_value[strval($x)]=0;
    }

    for($x = 0; $x < $num_row_activity; $x++) {
        $row = mysqli_fetch_array($result_activity);
        $point += (int)$row['add_point'];
        $type = substr($row['point'],0,1);
        $count = (int)substr($row['point'],1);
        $puzzle_value[$type] += $count;
    }

    $add_column = '';
    $add_value = '';
    for($x = 0; $x < count($puzzle_value); $x++){
        $type = array_keys($puzzle_value)[$x];
        $add_column = $add_column . ",unused" . $type;
        $add_value = $add_value . ",'".$puzzle_value[$type]."'";
    }

    $query_register_map = "INSERT INTO map (userid,emergency_list,point".$add_column.") VALUE ('$id','$emergency','$point'".$add_value.")";
    return $query_register_map;
}
...
function dealing_activity_request($mysqli,$login_certification){
    $query = "SELECT activity.number, activity.point
     FROM user, activity WHERE user.validation='$login_certification' AND user.userid = activity.userid AND activity.number != 'AB01'";
    $result = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
    $num_row = mysqli_num_rows($result);
...
function dealing_join_team_request($mysqli,$login_certification,$team_id){
    ...
    $query_check_user = "SELECT user.userid,used,teamid FROM map,user WHERE user.validation='$login_certification' AND user.userid = map.userid";
    ...
    if($row_check_user['teamid']!=NULL){
        $respond['error'] = 'already_in_team';
        echo json_encode($respond);
        return;
    }
...
function dealing_rank_request($mysqli,$rank_type){
    ...
    if($rank_type == 'person'){
        $query = "SELECT name , nickname, number ,department , title_use ,rank ,point FROM user, map WHERE user.userid=map.userid AND rank!=-1 ORDER BY rank LIMIT 10";
        $result = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
        $num_row = mysqli_num_rows($result);
        for($x = 1;$x <= $num_row; $x++){
            $row = mysqli_fetch_array($result);
        ...
        */
        ...
        $query = "SELECT name , point FROM team WHERE rank!=-1 ORDER BY rank LIMIT 10 ";
        $result = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
        $num_row = mysqli_num_rows($result);
        for($x = 1;$x <= $num_row; $x++){
            $row = mysqli_fetch_array($result);
