...
function dealing_create_team_request($mysqli,$login_certification,$team_name){
    ...
    if($num_row >= 1){
        if($row["teamid"] == -1){ //create team
            $team_num = time().rand(111111,999999);
            ...
            $query_map = "SELECT map.used FROM user, map WHERE user.validation='$login_certification', user.userid = map.userid";
