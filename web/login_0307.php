...
function dealing_rank_request($mysqli,$rank_type){
    ...
    if($rank_type == 'person'){
        for($x = 0; $x < 10; $x++) {
            $query = "SELECT userid, name , nickname, number ,department , title_use 
             FROM user WHERE rank='$x'";
            $result = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
            $num_row = mysqli_num_rows($result);
            if($num_row > 0){
                $row = mysqli_fetch_array($result);
                $respond['rank'.($x+1).'userid'] = $row['userid'];
                $respond['rank'.($x+1).'name'] = $row['name'];
                $respond['rank'.($x+1).'nickname'] = $row['nickname'];
                $respond['rank'.($x+1).'number'] = $row['number'];
                $respond['rank'.($x+1).'department'] = $row['department'];
                $respond['rank'.($x+1).'title_use'] = $row['title_use'];
            }          
        }
        $respond['sucess'] = 'true';
    }else if($rank_type == 'team'){
        for($x = 0; $x < 10; $x++) {
            $query = "SELECT teamid, name , point FROM team WHERE rank='$x'";
            $result = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
            $num_row = mysqli_num_rows($result);
            if($num_row > 0){
                $row = mysqli_fetch_array($result);
                $respond['rank'.($x+1).'teamid'] = $row['teamid'];
                $respond['rank'.($x+1).'name'] = $row['name'];
                $respond['rank'.($x+1).'point'] = $row['point'];
            }          
        }
        $respond['sucess'] = 'true';
    }
